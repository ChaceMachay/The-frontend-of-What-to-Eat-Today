...
import {
  getDishInformationByPage,
  getDishInformationBySearch,
} from "../test/api/dish";
...
const nextPage = async () => {
  try {
    dishInformation.value = getDishInformationByPage(
      nowPage.value + 1,
      qty.value
    );
  } catch {
...
const search = async () => {
  ...
  try {
    dishInformation.value = getDishInformationBySearch(
    ...
    <el-main>
      <el-table :data="dishInformation">
        ...
        <el-table-column label="地点" grow="1">
          <template #default="scope">
            {{ scope.row.dish_id.slice(0, 1) }}-{{
              scope.row.dish_id.slice(1, 2)
            }}-{{ scope.row.dish_id.slice(2, 4) }}-{{
              scope.row.dish_id.slice(4, 6)
            }}-{{ scope.row.dish_name }}
          </template>
        </el-table-column>
    ...
    <el-footer>
      ...
      >
        <div style="margin: 0 auto">
          <el-button link @click="lastPage">上一页</el-button>
          &ensp;
          <el-button link> 第 {{ nowPage }} 页 </el-button>
          &ensp;
          <el-button link @click="nextPage">下一页</el-button>
        </div>
      </div>
    </el-footer>
  </el-container>
